...
export default async function handler(req: any, res: any) {
  ...
  if (!code || typeof code !== "string") {
    res.setHeader("Content-Type", "text/html; charset=utf-8");
    res.status(400).send(renderSpotifyCallbackHtml({ error: "Missing Spotify authorization code." }));
    return;
  }
  ...
  try {
    ...
    res
      .status(500)
      .send(renderSpotifyCallbackHtml({ error: exchangeError.message || "Spotify token exchange failed." }));
  }
}
