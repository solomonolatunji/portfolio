<template>
  <form @submit.prevent="handleSubmit" class="space-y-6 pb-12">
    ...
    <div class="flex items-center justify-between">
      <div>
        <h1 class="bg-linear-to-r from-white to-gray-400 bg-clip-text text-3xl font-bold text-transparent">
      ...
      <div class="flex items-center gap-3">
        ...
        <Button variant="primary" size="md" :text="isEditing ? 'Update Project' : 'Publish Project'"
    ...
    <div class="grid grid-cols-1 gap-6 lg:grid-cols-[1fr_360px]">
      ...
      <div class="space-y-6">
        ...
        <div class="space-y-6 rounded-2xl border border-white/5 bg-white/5 p-6 transition-all hover:border-white/20">
          <div class="space-y-2">
            <label class="text-xs font-semibold tracking-widest text-gray-500 uppercase">Project Title</label>
            <input v-model="form.title" type="text" placeholder="Enter project name..." required
          ...
          <div class="grid grid-cols-2 gap-6 border-t border-white/5 pt-6">
            <Input v-model="form.year" label="Development Year" placeholder="e.g. 2024" required
        ...
        <div class="space-y-6">
          <div class="overflow-hidden rounded-2xl border border-white/5 bg-white/5">
            ...
            <div class="p-6">
              <textarea v-model="form.detailedDescription"
                ...
                class="min-h-[200px] w-full resize-y border-none bg-transparent text-lg leading-relaxed text-gray-200 placeholder-white/10 outline-none focus:ring-0"></textarea>
            </div>
          </div>
          ...
          <div class="overflow-hidden rounded-2xl border border-white/5 bg-white/5">
            ...
            <div class="p-6">
              <textarea v-model="form.challenges" placeholder="Detail the technical milestones (HTML supported)..."
                class="min-h-[200px] w-full resize-y border-none bg-transparent text-lg leading-relaxed text-gray-200 placeholder-white/10 outline-none focus:ring-0"></textarea>
            </div>
          </div>
          ...
          <div class="rounded-2xl border border-white/5 bg-white/5 p-6">
            ...
            <div class="space-y-4">
              <div class="grid grid-cols-3 gap-4 md:grid-cols-4 lg:grid-cols-5" v-if="preview.gallery.length > 0">
                <div v-for="(img, index) in preview.gallery" :key="index"
              ...
              <label for="gallery-upload"
                ...
                <input type="file" accept="image/*" multiple @change="handleGalleryUpload" id="gallery-upload"
                  class="hidden" />
              </label>
            </div>
          </div>
      ...
      <div class="space-y-6">
        ...
        <div class="space-y-6 rounded-2xl border border-white/5 bg-white/5 p-6 backdrop-blur-xl">
          ...
          <div class="space-y-4">
            <Input v-model="form.demoUrl" label="Live Preview" placeholder="https://"
              class="border-white/5! bg-transparent!" />
            <Input v-model="form.codeUrl" label="Source Repository" placeholder="https://"
              class="border-white/5! bg-transparent!" />
            <Input v-model="form.googlePlayUrl" label="Play Store" placeholder="https://"
              class="border-white/5! bg-transparent!" />
        ...
        <div class="rounded-2xl border border-white/5 bg-white/5 p-6 backdrop-blur-xl">
          ...
          <div class="space-y-4">
            <Input v-model="newTech" placeholder="Press Enter to add tech..." @keydown.enter.prevent="addTech"
            ...
            <div class="flex flex-wrap gap-2">
              <span v-for="(tech, index) in form.technologies" :key="index"
                ...
                {{ tech }}
                <button @click="removeTech(index)" class="ml-1.5 transition-colors hover:text-white">
          ...
          <div class="space-y-4">
            <Input v-model="newCategory" placeholder="Add category..." @keydown.enter.prevent="addCategory"
              class="border-white/5! bg-transparent! shadow-none focus:border-purple-500/50!" />
            <div class="flex flex-wrap gap-2">
              <span v-for="(cat, index) in form.categories" :key="index"
                ...
                {{ cat }}
                <button @click="removeCategory(index)" class="ml-1.5 transition-colors hover:text-white">
                  <IconX class="h-3.5 w-3.5" />
                </button>
              </span>
            </div>
        ...
        <div class="rounded-2xl border border-white/5 bg-white/5 p-6 backdrop-blur-xl">
          ...
          <div class="space-y-4">
            <Input v-model="newFeature" placeholder="Add a standout feature..." @keydown.enter.prevent="addFeature"
              class="border-white/5! bg-transparent! shadow-none focus:border-green-500/50!" />
            <div class="flex flex-wrap gap-2">
              <span v-for="(feat, index) in form.features" :key="index"
                ...
                {{ feat }}
                <button @click="removeFeature(index)" class="ml-1.5 transition-colors hover:text-white">
                  <IconX class="h-3.5 w-3.5" />
                </button>
              </span>
            </div>
          ...
          <div class="space-y-3">
            <div
              ...
              <label v-else for="main-upload"
                ...
                <input type="file" accept="image/*" @change="handleImageUpload" id="main-upload" class="hidden" />
              </label>
            </div>
          </div>
        </div>
