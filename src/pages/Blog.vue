          ...
          <div
            ...
            <div class="flex flex-col md:flex-row">
              <div class="relative w-full md:w-1/2">
                ...
                <div class="absolute inset-0 bg-gradient-to-t from-black/70 to-transparent md:bg-gradient-to-r"></div>
              ...
              <div class="flex w-full flex-col justify-between p-6 md:w-1/2 md:p-8">
                <div>
                  <div class="mb-4 flex flex-wrap items-center gap-2">
                    ...
                    <span class="rounded-full bg-[#2d2d2d] px-3 py-1 text-xs font-medium text-gray-300">
        ...
        <div>
          <div class="mb-6 flex items-center justify-between">
            ...
            <div class="flex gap-2">
              <button @click="setViewMode('grid')" class="rounded-md p-2" :class="viewMode === 'grid'
                ? 'bg-[#2d2d2d] text-white'
                : 'text-gray-400'
                ">
                <Squares2X2Icon class="h-5 w-5" />
              </button>
              <button @click="setViewMode('list')" class="rounded-md p-2" :class="viewMode === 'list'
                ? 'bg-[#2d2d2d] text-white'
                : 'text-gray-400'
                ">
          ...
          <div>
              ...
              <div v-for="article in paginatedArticles" :key="article.id"
                ...
                <div class="relative">
                  ...
                  <div class="absolute top-3 right-3">
                    <span class="rounded-md bg-[#2d2d2d]/80 px-2 py-1 text-xs text-gray-300 backdrop-blur-sm">
                      {{ article.category }}
                    </span>
                  </div>
                </div>
                <div class="p-5">
                  ...
                  <router-link :to="'/blog/' + article.id">
                    <h3 class="mb-2 text-lg font-bold text-white transition-colors hover:text-purple-400">
                      {{ article.title }}
                    </h3>
                  </router-link>
            ...
            <!-- List View -->
            <div v-else-if="viewMode === 'list' && paginatedArticles.length > 0" class="mb-10 space-y-5">
              <div v-for="article in paginatedArticles" :key="article.id"
                ...
                <div class="flex flex-1 flex-col justify-between p-5">
                  ...
                  <div class="mt-4 flex items-center justify-between gap-4">
                    <div class="flex items-center">
                      <img :src="article.author.avatar" :alt="article.author.name" class="mr-2 h-6 w-6 rounded-full" />
                      <span class="text-xs text-gray-300">{{
                        article.author.name
                        }}</span>
                    </div>
            ...
            <!-- Empty State -->
            <div v-else class="flex flex-col items-center justify-center rounded-xl bg-[#1e1e1e] py-16 text-center">
