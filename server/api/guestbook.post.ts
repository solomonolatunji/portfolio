...
export default defineEventHandler(async (event) => {
  ...
  const recent = await db.select({ count: count() }).from(guestbookEntries)
    .where(and(
      eq(guestbookEntries.userId, user.id),
      gt(guestbookEntries.createdAt, new Date(Date.now() - 60 * 60 * 1000)),
    ));
  if (Number(recent[0]?.count || 0) >= 5) throw createError({ statusCode: 429, statusMessage: "You can leave up to five messages per hour." });
